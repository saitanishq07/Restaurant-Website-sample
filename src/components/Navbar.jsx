    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled
            ? 'py-3 bg-ember-black/95 backdrop-blur-md border-b border-ember-gold/15 shadow-2xl'
            : 'py-6 bg-gradient-to-b from-ember-black/90 to-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 sm:px-8 flex items-center justify-between">
          ...
          <Link
            to="/"
            className="group flex items-center space-x-2 text-ember-cream focus:outline-none"
          >
              ...
              return (
                <Link
                  ...
                  className={`relative text-xs font-sans font-medium tracking-[0.2em] transition-colors duration-300 py-1 ${
                    isActive ? 'text-ember-gold font-semibold' : 'text-ember-cream/80 hover:text-ember-cream'
                  }`}
                >
                  {link.name}
                  {isActive && (
                    <div className="absolute bottom-0 left-0 right-0 h-[1.5px] bg-ember-gold" />
                  )}
                </Link>
              );
            })}
      ...
      {/* Mobile Overlay Menu */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-40 lg:hidden bg-ember-black/98 backdrop-blur-2xl flex flex-col justify-between p-8 pt-28 transition-opacity duration-300">
          ...
          <div className="space-y-6 pt-6 border-t border-ember-gold/15">
            <button
              onClick={handleReserveClick}
